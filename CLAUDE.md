...
Rather than invoking `htlatex` or `make4ht` as wrappers, we call `latex` directly, reproducing the tex4ht.sty injection preamble extracted from the htlatex script. The config string (`html5,ximera,charset=utf-8,-css`) is embedded via `\RequirePackage[config,html]{tex4ht}` hooked into `\@documentclasshook`:

```
latex -recorder -interaction=nonstopmode -shell-escape -file-line-error \
  "\makeatletter...\HCode html5,ximera,charset=utf-8,-css.a.b.c.\input stem"
```
